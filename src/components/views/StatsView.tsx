...
const StatsView: React.FC = () => {
  ...
  const hadithStatusData = [
    { name: t('statusAcquis'), value: hadithStatusCounts.acquis || 0, color: '#22c55e' },
    ...
    { name: t('statusLu'), value: hadithStatusCounts.lu || 0, color: '#6b7280' },
    { name: t('statusNonLu'), value: hadithStatusCounts.non_lu || 0, color: '#e5e7eb' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="text-center">
          ...
          <CardContent>
            ...
            <p className="text-sm text-text-secondary mt-2">{t('hadithsMastered')}</p>
          </CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader><CardTitle>📊 {t('hadithStats')}</CardTitle></CardHeader>
        <CardContent>
      ...
      {noData ? (
        ...
        <>
          {weeklyReadingData.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>📖 {t('readingActivity')}</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          {weeklyRevisionData.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>🧠 {t('revisionActivity')}</CardTitle>
              </CardHeader>
              <CardContent>
                <div style={{ width: '100%', height: 300 }}>
                  <ResponsiveContainer>
                    <BarChart data={weeklyRevisionData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                      ...
                      <Tooltip contentStyle={{ background: 'var(--card-bg)', borderColor: 'var(--border-main)', borderRadius: '0.5rem' }} />
                      <Legend formatter={() => t('unitsRevised')} />
