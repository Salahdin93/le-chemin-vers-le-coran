...
const WhatsAppFloatButton: React.FC = () => {
    ...
    return (
        <button
            ...
            className="fixed bottom-5 right-5 z-50 w-16 h-16 bg-[#25D366] text-white rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110"
            aria-label="Partager la progression sur WhatsApp"
        >
...
const MainAppView: React.FC = () => {
  ...
  useEffect(() => {
    setIsAnimating(true);
    const timer = setTimeout(() => setIsAnimating(false), 0,300);
  ...
  if (!activeProfile) {
    return (
        <div className="flex flex-col items-center justify-center min-h-screen text-center p-4">
            ...
            <p className="text-xl">Erreur : Profil non trouvé.</p>
            <p>Veuillez recharger l'application ou sélectionner un profil.</p>
        </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-6">
      ...
      <ShareModal />
      <ReadjustmentModal />
      <EndOfGoalModal isOpen={false} onClose={() => {}} />
