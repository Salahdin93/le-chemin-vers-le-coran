                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <AnimatePresence mode="wait">
                        {activeProfile && (
                            ...
                            >
                                <span className="font-amiri text-sm md:text-lg drop-shadow-sm font-bold" style={{ direction: 'rtl', color: activeProfile.accentColor || 'var(--accent-color)' }}>
                            ...
                            <div className="flex items-center gap-2 md:gap-3 pl-2 md:pl-3 py-1 md:py-1.5 pr-1 md:pr-1.5 rounded-xl md:rounded-2xl bg-bg-secondary/50 border border-border-main/30 backdrop-blur-md">
                                ...
                                <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg md:rounded-xl accent-gradient flex items-center justify-center text-white shadow-lg shadow-accent-color/20 group cursor-pointer hover:scale-105 transition-all">
                                    <Avatar
                                        ...
                                        config={activeProfile.avatar || { gender: activeProfile.gender === 'female' ? 'female_hijab' : 'male_beard', skinTone: '#E0AC69' }}
                                    />
                                </div>
                            </div>
                        )}
            ...
            <AnimatePresence>
                {isLoggingOut && (
                    ...
                    >
                        ...
                        >
                            <h1
                                className="font-amiri text-5xl md:text-7xl drop-shadow-lg leading-relaxed font-bold"
                                style={{ direction: 'rtl', color: activeProfile?.accentColor || 'var(--accent-color)' }}
                            >
                                بَارَكَ ٱللَّهُ فِيكُمْ
                            </h1>
                            <p
                                className="font-amiri text-2xl md:text-4xl text-text-main opacity-80 leading-relaxed drop-shadow-sm"
                                style={{ direction: 'rtl' }}
                            >
                                وَٱلسَّلَامُ عَلَيْكُمْ وَرَحْمَةُ ٱللَّهِ
                            </p>
                        </motion.div>
                    </motion.div>
                )}
