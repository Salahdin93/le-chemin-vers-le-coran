...
export const SimpleCheckbox: React.FC<CheckboxProps> = ({ label, description, id, className, ...props }) => {
  return (
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-3">
          <input
            ...
            {...props}
          />
          <label htmlFor={id} className="text-text-main cursor-pointer font-semibold">
            {label}
          </label>
        </div>
        {description && <p className="text-xs text-text-secondary mt-1 ml-7">{description}</p>}
      </div>
    </div>
  );
};
...
export const ToggleSwitch: React.FC<CheckboxProps> = ({ label, description, id, className, checked, ...props }) => {
  return (
    <div className={clsx('p-3 bg-bg-main rounded-lg border border-border-main', className)}>
